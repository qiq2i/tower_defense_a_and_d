####塔防召唤怪物
##$(uuid)为召唤的玩家的UUID，$(mob_id)为召唤怪物的识别id

####宏存储
data remove storage qiq2i_map2 summoning_mob_temp 
##存储获取的数据
$data modify storage qiq2i_map2 summoning_mob_temp.uuid set value $(uuid)
$data modify storage qiq2i_map2 summoning_mob_temp.id set from entity @e[type=marker,limit=1,nbt={data:{uuid:$(uuid)}}] data.id
$data modify storage qiq2i_map2 summoning_mob_temp.color_name set from entity @e[type=marker,limit=1,nbt={data:{uuid:$(uuid)}}] data.color_name
